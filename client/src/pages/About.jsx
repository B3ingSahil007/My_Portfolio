    ...
    return (
        <>
            <div style={{ marginTop: '1cm' }} className="container row align-items-center">
                <div className="col">
                    <p>Welcome, <span className='custom-shadow' style={{ color: '#5479f7' }}>{user ? `${user.firstname} ${user.lastname}` : 'To Our Website'}</span> . . .</p>
                    <h1>Why Choose <span className='custom-shadow' style={{ color: '#5479f7' }}> Us </span>?</h1>
                ...
                <div>
                    ...
                    {/* <button style={{ border: '2px solid #5479f7' }} onClick={() => { toast(`${user.firstname} ${user.lastname}`) }} className="btn btn-dark mx-3">Show Name</button> */}
                </div>
                <InfoPannel />
            </div>
            <style>{`
